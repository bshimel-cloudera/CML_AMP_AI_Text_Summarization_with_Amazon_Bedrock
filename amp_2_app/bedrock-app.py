...
def clear_out():
    cleared_tuple = (gr.Textbox.update(value=""), gr.Textbox.update(
        value=""), gr.Textbox.update(value=""), gr.Textbox.update(value=""))
...
with gr.Blocks() as demo:
    with gr.Row():
        ...
        example_holder = gr.Textbox(
            visible=False, label="Input Text", value="example")
    with gr.Row():
        modelId = gr.Dropdown(label="Choose a Bedrock Model:",
                              choices=models, value='amazon.titan-text-premier-v1:0')
    with gr.Row():
        with gr.Column(scale=4):
            custom_instruction = gr.Textbox(
                label="Input your prompt instruction:", value=example_instruction)
            input_text = gr.Textbox(
                label="Input your text", placeholder="Insert some long text here...")
            example = gr.Examples(examples=[[example_instruction, "CML Documentation"]], inputs=[
                                  custom_instruction, example_holder])
        with gr.Column(scale=4):
            with gr.Accordion("Advanced Generation Options", open=False):
                max_new_tokens = gr.Slider(
                    minimum=0, maximum=4096, step=1, value=512, label="Max Tokens")
                temperature = gr.Slider(
                    minimum=0.01, maximum=1.0, step=0.01, value=0.5, label="Temperature")
                top_p = gr.Slider(minimum=0, maximum=1.0,
                                  step=0.01, value=1.0, label="Top P")
            with gr.Accordion("Bedrock API Request Details", open=False):
                instruction_prompt = gr.Code(label="Instruction Prompt", value=prompt_construction(
                    'amazon.titan-text-premier-v1:0'))
                input_format = gr.JSON(label="Input Format", value=display_format(
                    'amazon.titan-text-premier-v1:0'))
    ...
    summarize_btn.click(fn=summarize, inputs=[modelId, input_text, custom_instruction, max_new_tokens, temperature, top_p], outputs=output,
                        api_name="summarize")
    reset_btn.click(fn=clear_out, inputs=[], outputs=[
                    input_text, output, example_holder, custom_instruction], show_progress=False)
    modelId.change(fn=prompt_construction, inputs=[
                   modelId], outputs=instruction_prompt)
    modelId.change(fn=display_format, inputs=modelId, outputs=input_format)
    example_holder.change(fn=example_lookup, inputs=example_holder,
                          outputs=input_text, show_progress=False)
